...
let availableCoffee = 50; // en gramos
let availableSugar = 10; // en cucharaditas
function hasCups() {
    return Object.values(availableCups).some((cups) => cups > 0);
}
function decrementCupSize(size) {
...
function decrementCoffee(amount) {
...
function hasSugar(amount) {
    return availableSugar >= amount;
}
function decrementSugar(amount) {
...
function dispenseCoffee(cupSize, sugarAmount) {
    let coffeeAmount;
    switch (cupSize) {
        case "small":
            coffeeAmount = 10;
